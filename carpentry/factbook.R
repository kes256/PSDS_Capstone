...
pivoted <- '../data/clean_factbook.json' %>%
  read_file() %>%
  parse_json() %>%
  unlist() %>%
  enframe() %>%
...
cia_extras <- c('Kosovo' = 'OWID_KOS', 'European Union' = 'EU', 'Saint Martin' = 'MAF', 'Virgin Islands' = 'VIR')  
...
write_csv(cia_all, '../data/factbook.csv')
write_csv(spdf_fortified, '../data/map.csv')
